...
import { Box, Card, CardContent, Typography, TextField, Button, Alert, Divider, CircularProgress } from "@mui/material"
...
const [successModalOpen, setSuccessModalOpen] = useState(false)
  const validateForm = () => {
    ...
    if (resource.isPaid && !formData.transactionId?.trim()) {
      newErrors.transactionId = "Transaction ID is required for premium resources"
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    dispatch(applyToResource({ application: applicationData, resourceId: resource.id }))
        ...
        <CardContent sx={{ p: 4 }}>
          ...
          {!resource.isActive && (
            <Alert severity="warning" sx={{ mb: 3 }}>
              <Typography variant="body2">
                This resource is currently unavailable. You can still submit a request and we'll notify you when it
                becomes available.
              </Typography>
            </Alert>
          )}
          ...
          <form onSubmit={handleSubmit}>
            ...
            {resource.isPaid && (
              <>
                ...
                <Alert severity="info" sx={{ mb: 3 }}>
                  <Typography variant="body2">
                    Please complete payment first, then enter your transaction ID below to access the premium resource.
                  </Typography>
                </Alert>
                <TextField
                  fullWidth
