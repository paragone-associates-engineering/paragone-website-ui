...
export const fetchEvents = createAsyncThunk('events/fetchEvents', async (params: EventsQueryParams = {}, { rejectWithValue }) => {
  try {
    const response = await axios.get<EventsResponse>(`${API_BASE_URL}/event/get-events`, {
      params: {
        page: 1,
        pageSize: 1000,
      },
...
export const fetchEventById = createAsyncThunk('events/fetchEventById', async (eventId: string, { rejectWithValue }) => {
  try {
    const response = await axios.get<EventsResponse>(`${API_BASE_URL}/event/get-events`);
...
export const applyToEvent = createAsyncThunk(
  ...
  async ({ eventId, application }: { eventId: string; application: EventApplication }, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/event/apply/${eventId}`, application);
...
export const { setCurrentPage, setFilters, clearSelectedEvent, clearApplicationSuccess, clearError } = eventsSlice.actions;
